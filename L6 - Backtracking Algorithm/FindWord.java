	...
	public boolean findWord(int r, int c, String word, int loc, char [][] bo)
	{
		...
		else  	// current character matches
		{
			...
			{		// Try all four directions if necessary.
				...
				// NEW
				// Adding the diagonals to this search is actually straightforward.
				// We simply make for new conditional calls, passing in the correct
				// arguments for the row and column to add the four new directions
				// to our search.
				if (!answer)
					answer = findWord(r-1, c+1, word, loc+1, bo);  // Up Right
				if (!answer)
					answer = findWord(r+1, c+1, word, loc+1, bo);  // Down Right
				if (!answer)
					answer = findWord(r-1, c-1, word, loc+1, bo);  // Up Left
				if (!answer)
					answer = findWord(r+1, c-1, word, loc+1, bo);  // Down Left										
